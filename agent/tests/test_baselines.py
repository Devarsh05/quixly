...
async def test_baseline_skips_completed_run_with_no_share_of_model(db, shop):
    ...
    chosen = await select_baseline_run(db, shop.id, before=PUBLISHED_AT)
...
async def test_baseline_is_scoped_to_one_shop(db, shop):
    ...
    assert await select_baseline_run(db, shop.id, before=PUBLISHED_AT) is None
...
async def test_explicit_baseline_id_is_validated_not_trusted(db, shop):
    ...
    assert (
        await select_baseline_run(db, shop.id, before=PUBLISHED_AT, baseline_run_id=good.id)
    ).id == good.id
    assert (
        await select_baseline_run(
            db, shop.id, before=PUBLISHED_AT, baseline_run_id=no_aggregate.id
        )
    ) is None
