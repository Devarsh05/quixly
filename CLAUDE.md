...
- **Compliance webhooks (Phase 5) — two are honest no-ops, one erases everything.** Shopify's three
  mandatory privacy webhooks are a submission gate: the automated review probes each endpoint,
  including with a deliberately invalid HMAC.
  - **`customers/data_request` and `customers/redact` are ACKNOWLEDGE-ONLY, and that is the accurate
    answer, not a shortcut.** Quixly stores **no end-customer personal data** — audited table by
    table across both schemas. `public` holds catalog + measurement rows keyed by shop; the query
    panels are generated by `graph/interrogator`, so even the engine queries are synthetic and no
    real shopper is involved. Prisma's `Session` carries name/email of **merchant staff**, not the
    merchant's customers. So the result set and the work set are both empty *by construction*, and
    neither handler calls the agent. **Never log the payload** — it carries the customer's id,
    email and phone, which would create the very record these handlers assert we do not keep. If
    Quixly ever ingests customer-linked data (orders, reviews, segments), both stop being no-ops.
  - **`shop/redact` erases via ONE statement, and that is a schema property.** `DELETE FROM shops`
    reaches all nine shop-scoped tables through `ON DELETE CASCADE`, so there is no manual delete
    order to get wrong. That is an *argument*; `tests/test_purge_shop.py` is the evidence — it
    asserts each of the nine tables individually, plus a second shop's rows surviving. A new table
    added without a CASCADE path back to `shops` fails that test, which is the point.
  - **An `active` shop is SKIPPED, loudly.** `shop/redact` lands 48h after uninstall and Shopify
    does not document it as cancelled on reinstall. Purging a live store would destroy its catalog
    and — irrecoverably — its verification baselines and uplift history, which cannot be recreated
    after the fact. The warning log is the signal that a genuine erasure went unhonoured.
  - **Enqueue, never delete inline.** Shopify wants a 2xx in seconds; a synchronous cross-service
    cascade that times out is a submission failure. The shell forwards → the dispatcher enqueues
    `purge_shop` → 204. The shell's own half (`shopify.Session` rows) is one local `deleteMany`,
    and it runs **after** a successful forward so a 500 leaves everything intact for the redelivery.
  - **Redis is cleared only when rows actually went.** A skipped reinstall keeps its cached token
    and ingest lock — dropping either would degrade a store we deliberately chose not to touch.
  - **Three rejection shapes, not two.** Invalid HMAC → **401** (the case the review sends, and the
    one Shopify's docs mandate); **missing** HMAC header → **400** (`MissingHeaders`, a separate
    branch of the library's `validate.ts`); valid → 200. Do not "fix" the 400 into a 401.
  - All three routes use `authenticateWebhookSerialized`, like every other webhook route:
    `customers/*` arrive while installed, so the library would otherwise refresh a near-expiry
    token outside the rotation lock. `shop/redact` normally has **no session** — that is fine, the
    HMAC uses the app secret and the wrapper already treats `ReauthRequiredError` as pass-through.
...
- [ ] Onboarding flow
- [x] **Mandatory compliance webhooks** — `customers/data_request`, `customers/redact`,
      `shop/redact`. Three app-shell routes + `agent/app/services/purge.py` +
      `agent/app/jobs/purge_shop.py` + a `SHOP_REDACT` dispatch branch. Registered via three
      `compliance_topics` subscriptions in `shopify.app.toml` (relative `uri`s; no scope change,
      no re-consent). Invariants in Conventions. **Not yet live** — needs `shopify app deploy` and
      a worker restart before the submission probe will pass.
